...
# Connection strings with embedded credentials (URL-style)
...
# SQL Server key-value connection strings with embedded passwords
if echo "$CONTENT" | grep -qiE '(Data Source|Server)=[^;]+;.*Password[[:space:]]*=[[:space:]]*[^;]{3,}'; then
  MATCHES="$MATCHES SQL Server connection string with password;"
fi

# Auth0 client secrets
if echo "$CONTENT" | grep -qiE '(auth0.*client.?secret|client.?secret.*auth0)[[:space:]]*[=:][[:space:]]*["\x27]?[A-Za-z0-9_\-]{20,}'; then
  MATCHES="$MATCHES Auth0 client secret;"
fi
