...
if echo "$COMMAND" | grep -qE 'dotnet[[:space:]]+nuget[[:space:]]+push'; then
  deny "Blocked: publishing NuGet packages should be done manually or via CI, not through Claude Code."
fi
