...
case "$FILE_PATH" in
  ...
  appsettings*.json|*/appsettings*.json)
    echo "{\"hookSpecificOutput\":{\"hookEventName\":\"PreToolUse\",\"permissionDecision\":\"ask\",\"permissionDecisionReason\":\"Editing appsettings — secrets must not be stored here. Use User Secrets (dev) or environment variables/Key Vault (prod). Confirm this change.\"}}"
    exit 2
    ;;
esac
